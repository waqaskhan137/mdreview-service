...
- [ ] **`attach_asset`** tool: args `{id (req), name (req), content_b64 (req)}` → maps to
...
- [ ] **`list_assets`** tool: args `{id (req)}` → maps to `GET /api/reviews/{id}/assets`, returns
      `{assets:[...]}`.
- [ ] Both are added to the `TOOLS` schema and the `route()`/dispatch mapping the same way the
      existing tools are (`create_review`, `get_feedback`, `update_source`, …) — name → (method,
      path, body), no new state in `mcp_server.py`.
- [ ] The `mcp_server.py` module docstring tool list is updated to include the two new tools.
- [ ] **GATING (mcp_smoke):** `mcp_smoke.py` (extended if it enumerates tools) shows `tools/list`
...
- [ ] Local validation passes: `python3 -m py_compile app.py` **and**
