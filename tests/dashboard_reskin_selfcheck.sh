...
node "$here/scripts/cdp-shot.mjs" --url "$url" --wait-for ".rw" --resize 1440x900 --wait 300 \
  ...
  --eval '(()=>{const n=document.querySelector("#noresults");if(getComputedStyle(n).display==="none")throw Error("noresults hidden on zero matches");const h=n.querySelector(".nr-h"),b=n.querySelector(".nr-body");if(!h||h.textContent!=="Nothing matches \"zzzz\"")throw Error("noresults heading: "+(h?JSON.stringify(h.textContent):"missing"));if(!b||!b.textContent.includes("Titles, projects and paths are searched"))throw Error("noresults body: "+(b?JSON.stringify(b.textContent):"missing"));return "NORESULTS ok"})()' \
