...
Tools map 1:1 to the API (18): `create_review` (with optional `project`/`session`/`source_path`
...
`get_comment`, `reply_to_comment`, `resolve_comment` (`justification?`), `delete_comment` (junk cleanup) — `document_id` is the
