...
class H(BaseHTTPRequestHandler):
    ...
    def route(self, m):
        ...
        if path == "/api/reviews" and m == "POST":
            ...
            #
            # #363: a latex create with no `markdown` and no `template` to seed one is ACCEPTED,
            # not rejected, and produces a review whose source is the empty string. That is
            # deliberate, decided against this exact ticket: _require_tex (latex_review/decorator.py)
            # passes allow_empty=True only on this arm, so starting a blank paper and filling it in
            ...
            # guard rejects an empty PUT once a paper exists, so nothing already written can be wiped
            # this way. Consequence for a caller: a 201 here is not proof the request body arrived.
            # #355 hit exactly this: a fixture posted its content under "source" instead of
            # "markdown", the unknown key was silently ignored, and a 201 with an empty document
