...
class LatexAwareReviews:
    ...
    def put_source(self, rid, markdown, expected_revision=None):
        # Signature widened WITH ReviewService.put_source (#288): this is an explicit override, not
        # __getattr__ delegation, so leaving it narrow would TypeError (500) every latex write the
        # moment the core arm passes expected_revision. The precondition compare itself lives in
        # the inner service, under the caller-held store.lock.
